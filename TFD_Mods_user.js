...
`;
		...
		const OBSERVERS = {
			...
			event_search: new MutationObserver(function(mutations) {
				mutations.forEach(function(mutation) {
					//console.log(mutation.addedNodes[0]);
					//console.log(VARIABLES.eventIsNotified);
					if (VARIABLES.eventIsNotified === false) {
						fn.API.testNotifyServerEvent();
					}
				});
			}),
		...
		const fn = { // all the functions for the script
			...
			backwork : { // backgrounds stuff
				...
				setupObservers() { // all the Observers that needs to run
					...
					OBSERVERS.event_search.observe(document.querySelector("#eventCountdown"), {
						childList: true,
					});
				},
			...
			API : { // the actual seeable and interactable part of the userscript
				...
				testNotifyServerEvent() {
					setTimeout(function() { VARIABLES.eventIsNotified = false }, 26 * 60 * 1000);
					VARIABLES.eventIsNotified = true;
					console.log('Event is notified to server');
				}
